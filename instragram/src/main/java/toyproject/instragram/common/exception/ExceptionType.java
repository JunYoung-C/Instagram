...
public enum ExceptionType {
    ...
    // 회원가입 관련 에러
    DUPLICATE_NICKNAME(new DuplicateNicknameException("다른 계정에서 이미 사용중인 사용자 이름입니다.")),
    DUPLICATE_EMAIL(new DuplicateEmailException("다른 계정에서 이미 사용중인 이메일입니다.")),
    DUPLICATE_PHONE_NUMBER(new DuplicateEmailException("다른 계정에서 이미 사용중인 휴대폰 번호입니다.")),
    INVALID_NICKNAME_BY_NUMBER(
            new InvalidNicknameByNumberException("사용자의 이름에 숫자만 포함할 수는 없습니다.")),
    INVALID_NICKNAME_BY_STRING(
            new InvalidNicknameByStringException("사용자 이름에는 문자, 숫자, 밑줄 및 마침표만 사용할 수 있습니다.")),
    INVALID_EMAIL(new InvalidEmailException("유효한 이메일 주소를 입력하세요.")),
    INVALID_PHONE_NUMBER(new InvalidPhoneNumberException("유효한 휴대폰 번호를 입력하세요."));
